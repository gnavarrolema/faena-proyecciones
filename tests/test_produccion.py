...
def test_simulacion_mortalidad_calcula_correctamente():
    """10,000 pollitos con 7.5% mortalidad = 9,250."""
    semanas = [_semana(date(2026, 1, 1), date(2026, 1, 7), 10000)]
    resultado = simular_mortalidad(semanas, tasas=[0.075])

    assert len(resultado) == 1
    sim = resultado[0]
    assert sim.pollitos_cargados == 10000
    assert len(sim.simulaciones) == 1
    assert sim.simulaciones[0].tasa_mortalidad == 0.075
    assert sim.simulaciones[0].pollitos_disponibles == 9250
...
def test_multiples_tasas_mortalidad():
    """Las 7 tasas por defecto deben generar 7 resultados distintos."""
    semanas = [_semana(date(2026, 1, 1), date(2026, 1, 7), 100000)]
    resultado = simular_mortalidad(semanas)

    assert len(resultado[0].simulaciones) == 7
    ...
    # Con 100,000 y tasas 4.5%, 5%, 5.5%, 6%, 6.5%, 7%, 7.5%:
    ...
    assert disponibles[5] == 93000  # 7.0%
    assert disponibles[6] == 92500  # 7.5%
...
def test_multiples_semanas():
    ...
    assert resultado[0].simulaciones[0].pollitos_disponibles == 9250
    assert resultado[1].simulaciones[0].pollitos_disponibles == 18500
    assert resultado[2].simulaciones[0].pollitos_disponibles == 13875
